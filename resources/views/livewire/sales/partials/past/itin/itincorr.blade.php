<div class="tab-pane fade" id="vert-tabs-itin-corr_past" role="tabpanel" aria-labelledby="vert-tabs-itin-corr-tab_past" wire:ignore.self>     
    <div class="col">
      <div class="card card-danger">
        ...
        <div class="card-body table-responsive p-0">
          <table class="table">
            ...
            @endforeach
            @endif
            {{-- @if(!empty($corrItinXlsx_past))
            @foreach($corrItinXlsx_past as $category)
            <tr>
            <td>{{$category->filename}}</td>
            <td>{{$category->filesize}} KB</td>
            <td>{{ Carbon\Carbon::parse($category->created_at)->format('m-d-Y') }}</td>
