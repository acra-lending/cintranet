...
class FFNonOwnerOccuUpload {   

    public function uploadFfNonOwnerOccu(Request $request)
    {
        $num = 2;
        //Handle File Upload
        if($request->hasFile('file')){
            ...
            foreach ($request->file('file') as $file){
                ...
                // Filename to store
                $fileNameToStore = 'NOO 3- ILD Credit Grids  Pricing '.date('m-d-Y').'.'.$extension;
                
                //Check if Filename exists
                while(Storage::disk('local')->exists('public/upload/'.$fileNameToStore)){
                    $fileNameToStore = 'NOO 3- ILD Credit Grids  Pricing '.date('m-d-Y').' v'.$num.'.'.$extension;
                    $num++;
                }
                // Upload
